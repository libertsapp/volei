...
6. **Quadro de campeões sem rotação (pedido do usuário, v9.7):** o quadro do Início deixa de virar: sai o verso ("Times de …"), a dica "toque no quadro…", o `wireDashFlipCard`, o timer de 5s, o CSS 3D (`.flip-card-*`) e o som de troféu que tocava no quadro. Fica só a frente fixa (equipe campeã em cartões e os 3 números do ano). Os times completos de cada rodada continuam no Histórico. Isso substitui os itens 4 e 5 acima (verso do quadro) e o item 1 destes ajustes (exceção do virar).
